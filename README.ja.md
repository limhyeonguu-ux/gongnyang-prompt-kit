...
「ポスターを1枚作って」程度のリクエストを受け取り、そのまま生成に投入できる完成プロダクションプロンプトを組み立てる。上のキービジュアルも、このキットでコンパイルしたプロンプト（C11 シネマティック・キーアート）から生成したものだ。コンパイル記録は韓国語ファーストだが、キットが教える文法（Scene / Camera / Lighting / Color grading / Texture、HEX 固定パレット、末尾の `AR` トークン1つ）は言語非依存で、日本語・英語プロンプトにもそのまま持ち込める。

![コンパイルデモ — ラフなリクエスト → 完成プロンプト → 検証器パス](docs/hero.gif)

ラフなリクエスト → 完成プロンプト → 検証器パス。この3ステップがスキルの中で完結する。画像生成そのものはスコープ外だ — 大量生成は [codex-fleet](https://github.com/kimsh-1/codex-fleet) の `codex-imagegen` スキル、1枚だけなら `codex` に直接投入する。

> インタラクティブデモ: **[kimsh-1.github.io/gongnyang-prompt-kit](https://kimsh-1.github.io/gongnyang-prompt-kit)**

## 違いはプロンプトだけ

同じ gpt-image-2 だ。**左は人間が打ったひと言（「ヤバい画像を1枚作って」）をそのまま入れた結果、右はそのひと言をキットがコンパイルして入れた結果。** コンパイルレコード全文は [`examples/showcase.jsonl`](examples/showcase.jsonl)。
...
v3 のコアはルーティング表ひとつだ。リクエストのシグナルを受けたら下の表から1行を選び、その行が指すリファレンスファイルを**1つだけ**読む。ルーティングの正本は [`skills/image-prompt/SKILL.md`](skills/image-prompt/SKILL.md) にある表の一箇所のみで、以下はその読者向けミラーだ。
...
Claude Code で「画像プロンプトを書いて」「グラビアプロンプト」「キーアート」「タイポポスター」のようなトリガー、または `/image-prompt` で実行する。シンボリックリンクでインストールすればリポジトリの更新が自動反映される。検証器の実行には Node.js が必要だ。

書いたプロンプトは検証器でチェックする。ティアを認識し、ホワイトリスト外のネガティブだけを捕まえる。
...
`{ok, format, tier, errors, warnings}` の JSON を返す。ホワイトリスト外のネガティブ・先頭ブラケット・SD 廃止語彙・サイズロック違反・スロットトークン残存は `error`（肯定形 rewrite ヒント付き）、空虚な形容詞・HEX 欠落などは `warning`。パス・失敗サンプルは [`examples/`](examples/) にある。

生成までつなぐには [Codex CLI](https://github.com/openai/codex) のログイン + ChatGPT Plus/Pro が必要だ。

## 構造

ラフなリクエストがスキルコアとリファレンスを経て完成プロンプトになり、検証器を通過してはじめて生成へ進む。SKILL.md には常時ロードされるコアだけを置き、深いディテールは `references/` に分離した（progressive disclosure）。
...
変更履歴・検証実測 → [GitHub Releases](https://github.com/kimsh-1/gongnyang-prompt-kit/releases)。リリースチェックリストは [`RELEASING.md`](RELEASING.md)、ライセンスは [MIT](LICENSE)。
