...
- **텍스트 정확도의 1차 레버는 캔버스 세로 높이.** 21:9(codex 경로 세로 ~810px)는 400자에서 글자가 작아진다. 16:9(~950px)·2:3로 세로를 확보하면 700~800자도 또렷하다. codex 경로는 큰 캔버스 요청을 장변 ~1900px로 정규화하므로, 정확도는 비율 선택으로 번다.
- **자유 작성 존이 밀도의 핵심 무기.** 크리티컬 라벨만 따옴표로 고정하고 본문은 `genuine Korean sentences, fully written in real hangul`로 위임하면, 모델이 개념적으로 맞는 설명을 스스로 채운다(B-트리 키값 분배·Raft 로그 인덱스까지 논리 정합).
- **시퀀스·다대다·피드백도 정면 돌파된다.** 관건은 모델 능력이 아니라 노드·연결·방향을 문장으로 얼마나 구체적으로 지정하느냐다.

> 💡 고밀도 텍스트 슬라이드는 세로 높은 비율(16:9·2:3)에서 가장 안정적이다. 제목·핵심 수치 같은 치명 카피는 따옴표로 고정하고, 본문 밀도는 자유 작성 존에 위임한 뒤 필요한 컷만 다시 뽑으면 된다.

## 홍보판촉물 그래픽 (P1~P8) — 디자이너 포스터 문법

카드뉴스 미감(정보성 SNS 카드)이 아니라 **실제 디자이너가 만든 홍보판촉물** 톤으로 뽑는 레이어. 디자이너 레퍼런스 14장에서 증류한 **8개 레이아웃 문법**(P1~P8) — 타이포가 장식이 아니라 피사체와 물리적으로 얽히는 구조 + 2~3색 하드 락 + 인쇄 마감 디바이스(바코드·크롭마크·에디션 넘버). 룩 프리셋(L)과 직교하며 서로 교배(크로스브리드)된다. 상세는 [`skills/image-prompt/references/promo-router.md`](skills/image-prompt/references/promo-router.md)(패턴별 드롭인은 [`references/promo/`](skills/image-prompt/references/promo/)).
...
한글 카피가 전 컷에서 깨끗하게 렌더된다 — 그림자 서사(H)는 오브제→그림자로 서사가 열리는 시네마틱 키아트에서 3어절 한글 슬로건까지 정확하다. 룩 프리셋 드롭인은 [`skills/image-prompt/references/look-presets.md`](skills/image-prompt/references/look-presets.md).
...
"있어보이게"는 감으로 조립하지 않는다 — `references/look-presets.md`의 **룩 프리셋 9종**(럭셔리 에디토리얼·시네마틱 그레이드·미니멀 프로덕트·스위스 타이포·다크 테크·레트로 인쇄·파스텔·골드 포일·홍대 인디)에서 골라 드롭인한다.
